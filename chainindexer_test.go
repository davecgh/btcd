...
func runAddrIndexTestCases(t *testing.T, db database.DB, tests []addrIndexTestCase) error {
	...
	err := db.View(func(dbTx database.Tx) error {
		for i, test := range tests {
			regions, skipped, err := dbFetchAddrIndexEntries(dbTx, test.key, test.numToSkip, test.numRequested, test.reverse)
			if err != nil {
...
var errAddrIndexTipMismatch = errors.New("Address index tip mismatch")

func checkAddrIndexTip(t *testing.T, db database.DB, wantHash *wire.ShaHash, wantHeight int32) error {
	// Check addrindex tip now points to the new block.
	err := db.View(func(dbTx database.Tx) error {
		hash, height, err := dbFetchAddrIndexTip(dbTx)
		if err != nil {
			return err
		}
		if *hash != *wantHash || height != wantHeight {
			t.Errorf("Addrindex tip mismatch: got %v %v, want %v %v",
				hash, height, wantHash, wantHeight)
			return errAddrIndexTipMismatch
		}
		return nil
	})
	if err != nil {
		t.Errorf("Failed to fetch addrindex tip: %v\n", err)
		return err
	}
	return nil
}
...
func TestAddrIndexStorage(t *testing.T) {
	...
	err = dbCreateAddrIndex(db, chaincfg.MainNetParams.GenesisHash)
	if err != nil {
		t.Errorf("Failed to create address index: %v\n", err)
		return
	}

	// Check creating the addr index sets the addrindex tip properly.
	if checkAddrIndexTip(t, db, chaincfg.MainNetParams.GenesisHash, 0) != nil {
		return
	}
	...
	err = db.Update(func(dbTx database.Tx) error {
		return appendAddrIndexDataForBlock(dbTx, testBlock1, testData1)
	})
	if err != nil {
		t.Errorf("Failed to append block to index: %v\n", err)
		return
	}
	if checkAddrIndexTip(t, db, testBlock1.Sha(), 1) != nil {
		return
	}
	...
	// Check calling dbCreateAddrIndex when the index is already created
	// works fine. It should not fail, but not reset the tip or change data.
	err = dbCreateAddrIndex(db, chaincfg.MainNetParams.GenesisHash)
	if err != nil {
		t.Errorf("Failed to re-create address index: %v\n", err)
		return
	}
	if checkAddrIndexTip(t, db, testBlock1.Sha(), 1) != nil {
		return
	...
	// Add the 2nd block
	err = db.Update(func(dbTx database.Tx) error {
		return appendAddrIndexDataForBlock(dbTx, testBlock2, testData2)
	})
	if err != nil {
		t.Errorf("Failed to remove block from index: %v\n", err)
		return
	}
	if checkAddrIndexTip(t, db, testBlock2.Sha(), 2) != nil {
		return
	}
	...
	err = db.Update(func(dbTx database.Tx) error {
		return removeAddrIndexDataForBlock(dbTx, testBlock2, testData2)
	})
	if err != nil {
		t.Errorf("Failed to remove block from index: %v\n", err)
		return
	...
	err = db.Update(func(dbTx database.Tx) error {
		return removeAddrIndexDataForBlock(dbTx, testBlock1, testData1)
	})
	if err != nil {
		t.Errorf("Failed to remove block from index: %v\n", err)
		return
